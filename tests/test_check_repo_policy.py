...
# Worker 模式（控制面设置了 manifest 变量）里刻意没有真实 Git。只有建临时 Git 仓库或读提交图的
# 用例在那里跳过，逐个标注、原因固定；CI 不设这个变量，全量运行。Worker 里的 skipped 不是 passed。
requires_git = unittest.skipIf(
    os.environ.get(policy.MANIFEST_ENV) is not None,
    "needs real Git: skipped only when ACUVEN_GIT_LS_FILES_MANIFEST is set (Worker mode has no Git)",
)
...
# --------------------------------------------------------------------------
# Worker 模式的只读 manifest：Worker 里没有 Git，控制面给的是 `git ls-files -z` 的原始输出。
# 读不懂就退出 2，绝不退化成「当作没有文件」—— 那等于策略检查静默全过。
# 下面的用例都不启动 Git，Worker 模式下照常运行。
# --------------------------------------------------------------------------

def no_subprocess():
    return patch.object(policy.subprocess, "run", side_effect=AssertionError("git must not run"))
...
BAD_MANIFESTS = {
    "empty file": b"",
    "missing final NUL": b"docs/bad.md",
    "newline framing": b"docs/bad.md\n",
    "empty record": b"docs/bad.md\0\0",
    "lone NUL": b"\0",
    "invalid UTF-8": b"docs/\xff.md\0",
    "overlong UTF-8": b"docs/\xc0\xae.md\0",
    "encoded surrogate": b"docs/\xed\xa0\x80.md\0",
    "byte order mark": b"\xef\xbb\xbfdocs/bad.md\0",
    "absolute": b"/etc/passwd\0",
    "drive letter": b"C:/docs/bad.md\0",
    "backslash": b"docs\\bad.md\0",
    "traversal": b"../outside.md\0",
    "inner traversal": b"docs/../bad.md\0",
    "dot segment": b"./docs/bad.md\0",
    "inner dot segment": b"docs/./bad.md\0",
    "double slash": b"docs//bad.md\0",
    "trailing slash": b"docs/\0",
    "trailing dot": b"docs/bad.md.\0",
    "trailing space": b"docs/bad.md \0",
    "tab": b"docs/bad\t.md\0",
    "embedded newline": b"docs/bad.md\nother.md\0",
    "DEL": b"docs/bad\x7f.md\0",
    "C1 control": "docs/bad\x85.md\0".encode("utf-8"),
    "duplicate": b"docs/bad.md\0README\0docs/bad.md\0",
}
...
class ManifestTests(unittest.TestCase):
    def setUp(self):
        ...
        self.tmp = os.path.normpath(os.path.abspath(tmp.name))
        self.root = Path(self.tmp) / "repo"
        (self.root / "docs").mkdir(parents=True)
        (self.root / "docs" / "bad.md").write_text("- [~] manifest 里列出的文件要查\n", encoding="utf-8")
        (self.root / "docs" / "stray.md").write_text("- [~] manifest 没列出的文件不查\n", encoding="utf-8")

    def manifest(self, data: bytes, name: str = "manifest.bin") -> str:
        path = os.path.join(self.tmp, name)
        with open(path, "wb") as handle:
            handle.write(data)
        return path

    def as_root(self, value: str):
        """以临时目录冒充仓库根（ROOT），并把 manifest 变量指向 value。"""
        return patch.dict(os.environ, {policy.MANIFEST_ENV: value}), patch.object(policy, "ROOT", self.root)

    def check(self, value: str) -> list[str]:
        env, root = self.as_root(value)
        with env, root, no_subprocess() as run:
            errors = policy.check_local()
        run.assert_not_called()
        return errors

    def run_main(self, value: str) -> tuple[int, str]:
        env, root = self.as_root(value)
        stderr = io.StringIO()
        with env, root, no_subprocess(), redirect_stdout(io.StringIO()), redirect_stderr(stderr):
            code = policy.main([])
    ...
    def assert_rejected(self, value: str) -> None:
        with self.assertRaises(policy.PolicyError):
            self.check(value)
        code, stderr = self.run_main(value)
        self.assertEqual(code, 2, stderr)
        self.assertIn("CHECK ERROR", stderr)
        # 报错不回显本机路径，也不回显记录内容
        self.assertNotIn(self.tmp, stderr)
        self.assertNotIn("bad.md", stderr)

    def test_manifest_is_the_file_list_and_git_never_runs(self):
        value = self.manifest(b"docs/bad.md\0docs/missing.md\0README\0")
        self.assertEqual(self.check(value), ["docs/bad.md:1: unsupported task checkbox [~]"])

    def test_records_are_strict_utf8(self):
        (self.root / "docs" / "中文 说明.md").write_text("- [?] x\n", encoding="utf-8")
        value = self.manifest("docs/中文 说明.md\0".encode("utf-8"))
        self.assertEqual(self.check(value), ["docs/中文 说明.md:1: unsupported task checkbox [?]"])

    def test_violation_is_exit_1_and_clean_manifest_is_exit_0(self):
        self.assertEqual(self.run_main(self.manifest(b"docs/bad.md\0"))[0], 1)
        self.assertEqual(self.run_main(self.manifest(b"README\0docs/missing.md\0"))[0], 0)

    def test_malformed_manifest_contents_fail_closed(self):
        for name, data in BAD_MANIFESTS.items():
            with self.subTest(name):
                self.assert_rejected(self.manifest(data))

    def test_size_limit_is_1000000_bytes(self):
        at_limit = b"a" * (policy.MANIFEST_MAX_BYTES - 1) + b"\0"
        self.assertEqual(self.check(self.manifest(at_limit)), [])
        self.assert_rejected(self.manifest(b"a" * policy.MANIFEST_MAX_BYTES + b"\0"))

    def test_growing_after_lstat_is_still_caught(self):
        value = self.manifest(b"a" * policy.MANIFEST_MAX_BYTES + b"\0")
        small = SimpleNamespace(st_mode=stat.S_IFREG | 0o444, st_size=2)
        with patch.object(policy.os, "lstat", return_value=small):
            self.assert_rejected(value)

    def test_variable_must_be_an_absolute_normalized_path(self):
        # 文件真实存在：拒绝的是写法本身，不是「找不到」
        self.manifest(b"docs/bad.md\0")
        for bad in ("", "manifest.bin", os.path.join(self.tmp, "sub", "..", "manifest.bin")):
            with self.subTest(bad=bad):
                self.assert_rejected(bad)

    def test_missing_manifest_and_directory_fail_closed(self):
        self.assert_rejected(os.path.join(self.tmp, "absent.bin"))
        self.assert_rejected(self.tmp)

    def test_link_and_reparse_point_fail_closed(self):
        # 在 Windows 上建真链接要特权，用 lstat 的返回值表达「它是链接 / reparse point」
        value = self.manifest(b"docs/bad.md\0")
        link = SimpleNamespace(st_mode=stat.S_IFLNK | 0o777, st_size=12)
        reparse = SimpleNamespace(
            st_mode=stat.S_IFREG | 0o444, st_size=12, st_file_attributes=stat.FILE_ATTRIBUTE_REPARSE_POINT,
        )
        for name, info in (("link", link), ("reparse point", reparse)):
            with self.subTest(name), patch.object(policy.os, "lstat", return_value=info):
                self.assert_rejected(value)

    def test_unreadable_manifest_fails_closed(self):
        value = self.manifest(b"docs/bad.md\0")
        with patch.object(policy.os, "lstat", side_effect=PermissionError(13, "denied", value)):
            self.assert_rejected(value)
        with patch.object(policy, "open", create=True, side_effect=PermissionError(13, "denied", value)):
            self.assert_rejected(value)


class GitSourceTests(unittest.TestCase):
    """用假的 subprocess.run 证明走的是哪条路；不启动 Git，Worker 模式下照常运行。"""

    def test_without_variable_check_local_calls_git_ls_files(self):
        with patch.dict(os.environ), fake_git("docs/a.txt\0") as run:
            os.environ.pop(policy.MANIFEST_ENV, None)
            self.assertEqual(policy.check_local(), [])
        run.assert_called_once()
        self.assertEqual(run.call_args.args[0], [
            "git", "-C", str(policy.ROOT), "ls-files", "-z", "--cached", "--others", "--exclude-standard",
        ])

    def test_temporary_root_ignores_the_manifest(self):
        # 变量指向一个不存在的 manifest：要是被读了，这里就会是 PolicyError
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            root = Path(tmp)
            absent = os.path.join(os.path.normpath(os.path.abspath(tmp)), "absent.bin")
            with patch.dict(os.environ, {policy.MANIFEST_ENV: absent}), fake_git() as run:
                self.assertEqual(policy.check_local(root), [])
        self.assertEqual(run.call_args.args[0], ["git", "-C", str(root), *policy.LS_FILES_ARGS])

    def test_pr_body_and_response_checks_still_use_git(self):
        sha = "a" * 40
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            root = Path(tmp)
            manifest = os.path.join(os.path.normpath(os.path.abspath(tmp)), "manifest.bin")
            with open(manifest, "wb") as handle:
                handle.write(b"README\0")
            body = root / "b.md"
            body.write_text(NONE_BODY, encoding="utf-8")
            resp = root / "r.md"
            resp.write_text(response("| 阻断 1 | 已修 | 已写进收口条件 | 看过了 |", sha), encoding="utf-8")
            cases = (
                (["--body-file", str(body), "--base", sha, "--head", sha], 0,
                 ["diff", "--name-only", "--no-renames", "-z", f"{sha}...{sha}"]),
                (["--response-file", str(resp), "--head", sha], 1,
                 ["merge-base", "--is-ancestor", sha, sha]),
            )
            for argv, expected, command in cases:
                with self.subTest(argv[0]), patch.dict(os.environ, {policy.MANIFEST_ENV: manifest}), \
                        patch.object(policy, "ROOT", root), fake_git() as run, \
                        redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
                    self.assertEqual(policy.main(argv), expected)
                    commands = [call.args[0][3:] for call in run.call_args_list]
                    self.assertIn(command, commands)
                    # 本地检查仍走 manifest：没有一次 ls-files
                    self.assertNotIn(list(policy.LS_FILES_ARGS), commands)


class ManifestMatchesGitTests(unittest.TestCase):
    @requires_git
    def test_real_ls_files_output_parses_record_for_record(self):
        # 控制面写的就是这条命令的原始字节；本仓库自己的输出必须能被逐条读懂
        raw = subprocess.run(
            ["git", "-C", str(policy.ROOT), *policy.LS_FILES_ARGS], capture_output=True, check=True,
        ).stdout
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            manifest = os.path.join(os.path.normpath(os.path.abspath(tmp)), "manifest.bin")
            with open(manifest, "wb") as handle:
                handle.write(raw)
            records = policy.read_manifest(manifest)
            with patch.dict(os.environ, {policy.MANIFEST_ENV: manifest}), no_subprocess():
                via_manifest = policy.check_local()
        self.assertEqual(records, [record for record in raw.decode("utf-8").split("\0") if record])
        with patch.dict(os.environ):
            os.environ.pop(policy.MANIFEST_ENV, None)
            self.assertEqual(via_manifest, policy.check_local())
