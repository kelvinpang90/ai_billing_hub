...
CONSISTENCY_RULES = [
    ...
    {
        # 分档表第二行同样四处各有一份（WORKFLOW、design-gate 模板、审查清单、
        # codex-review 的提示词）。2026-09-12 把「集成认证」扩成「认证与会话
        # （集成侧与平台侧）」时才发现：第四行有约定串钉着，第二行没有 ——
        # 只改一处就会悄悄漂移，而这正是第四行当初被钉住的原因。
        #
        # ⚠️ loose 必须锚在**不会变的两端**（「用量摄取」与「Webhook」），中间放宽。
        # 初版锚在「认证与会话」上，结果把中间整词改回「集成认证」时正则干脆不匹配，
        # 规则对这种漂移视而不见 —— 变异测试当场抓到，这正是它要防的那种改法。
        #
        # 负向后顾排除第四行那句否定式（「……也不碰用量摄取 / 认证与会话 / Webhook
        # 的后端改动」），否则那一句会被当成本条的不一致写法，永远红。
        "name": "走闸门的第二档主题",
        "canonical": "用量摄取 / 认证与会话（集成侧与平台侧）/ Webhook",
        "loose": re.compile(r"(?<!也不碰)用量摄取\s*/\s*[^\n|]{0,20}?/\s*Webhook"),
    },
    {
        # 与上一条同源：闸门分档表在 WORKFLOW、design-gate 模板、PR 模板、
        # CLAUDE.md、审查清单五处各有一份。2026-09-11 补的第四档要是只改一处，
        # Codex 审查时读到的还是旧表 —— PR #22 第三轮就是这么误判的。
        "name": "不走闸门的其余后端改动",
        "canonical": "既不碰钱、也不碰用量摄取 / 认证与会话 / Webhook 的后端改动",
        # 排除 `[` 是为了让这条规则不匹配它自己这行正则字面量（上面那几条
        # 靠 `\s*` 天然不自匹配，这条的字符类不然）。
        "loose": re.compile(r"既不碰钱[^\n\[]{0,60}?后端改动"),
    },
    {
