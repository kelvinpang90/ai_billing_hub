...
> **这不是需求正文。** 正文在 [Acuven_Central_AI_Billing_Platform_Spec_v1.1.md](Acuven_Central_AI_Billing_Platform_Spec_v1.1.md)（140 节）。
...
评审 P2-24 指出：spec 有 140 个一级标题、层级偏平，且**只有 12 个 `REQ-*` ID**，而 §132 的 Definition of Done 要求逐条验收。

- [ ] 为每条硬性要求补编号，使代码 / PR 可引用「实现 REQ-xxx」
- [ ] 修复 §3.2 / §3.3 的标题层级（被写成了一级标题）

这两项**不阻塞** Phase 0，但会影响 §132 的可验收性，建议在 Phase 1 之前处理。

## 四、评审意见的落实状态

[SPEC_REVIEW_v1.0.md](SPEC_REVIEW_v1.0.md) 对 v1.0 提了 25 条（P0×7 / P1×10 / P2×8）。spec v1.1 的 Revision History 声称已澄清「货币转换、定价数据、异步摄取、幂等、财务期间、状态处理、安全、恢复、合规闸门、生产运维」。

⚠️ **逐条核对尚未做过。** 从章节结构看，P0 的 7 条都有了对应新章节（§17.1 汇率、§74.4 凭据加密、§20 异步摄取、§74.1/§74.3 定价表字段、§98.1 备份恢复），但「有章节」不等于「写对了」。

- [ ] 逐条比对 25 条评审意见与 v1.1 正文，产出核对结论
