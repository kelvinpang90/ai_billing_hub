...
⚠️ **用 `-Reviewer claude` 时这条缺口是真实存在的**：审查者与实现方同属 Claude。冷读（全新会话、只看材料）能挡住「记得自己写过什么」，挡不住同一模型的共同盲点。所以每条这样的审查评论都在署名前缀下注明了实际审查者；碰钱的设计与实现在 Codex 恢复后，值得再用 Codex 复看一次关键决定。
