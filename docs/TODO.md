...
- [ ] **R7 — 对账单 cut-off 是 T+1 还是 T+3**：[ADR-0003](adr/ADR-0003-financial-period-and-cutoff.md) 里「T+3 也挡不住 31 小时积压」这条论据是错的（T+3 = 72 小时，覆盖得住），已撤回。撤回后 T+1 的论证强度下降，需重新判断。**Phase 7 前必须定，但越早越好**——它决定 `monthly_statements` 的生成时点与客服话术。（来源：Codex 审查 PR #4）
