...
✅ **升配到 2 核之后在生产机上重跑过，结论复核通过**（2026-09-17，[perf-baseline.md](perf-baseline.md) §9）：
写路径仍在并发 8 之后饱和；而且**只要 api 的 CPU 配额 ≤ 1 核，加 `--workers` 就不可能提高吞吐**
—— 多个进程分的是同一份配额。⚠️ 重跑顺带发现当时的限额（api 0.5 核）才是最大瓶颈：
一次登录 ~600 ms。限额已调高，见 §3.3。重新评估的触发条件：Phase 2 用量摄取上线。
...
| 服务 | mem_limit | cpus |
| --- | ---: | ---: |
| mysql | ~~512m~~ **1g** | ~~0.6~~ **0.8** |
| api | ~~384m~~ **768m** | ~~0.5~~ **1.0** |
| celery-worker | ~~384m~~ **768m** | ~~0.5~~ **1.0** |
...
⚠️ **2026-09-17 调高**（升配到 2 核 / 7.4 GB 之后第一次调）。依据是生产机上的两轮容量基线
（[perf-baseline.md](perf-baseline.md) §9）：同一台机器上，api 从 0.5 核到 1.0 核，一次登录从
**596 ms 降到 279 ms**，写路径峰值从 **~130/s 到 308/s**，`/healthz` 从 ~415/s 到 ~674/s。
合计 2.9 GB 的**上限**（不是预留）对 7.4 GB 的机器是安全的，同机其它项目实测占用约 3 GB。
