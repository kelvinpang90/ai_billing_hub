...
⚠️ **顺带查清了磁盘现状**：根文件系统 29 GB，已用 19 GB（**68%**），离 80% 的警戒线只剩约 3.5 GB。
占大头的是 `/var/lib/docker` 的 9.6 GB —— 本项目的镜像有 **16 个**（8 代部署 × api + frontend，
每代约 420 MB），**没有任何清理**：`deploy.sh` 只保留上一代用于回滚，更老的一直留着。
按现在的部署频率，**磁盘告警最先响的原因多半就是它**。⚠️ 清理不能用 `docker image prune -a`：
同一台机器上还跑着另外八个项目，那条命令会把它们的镜像一起删掉 —— 只能按本项目的两个仓库名、
保留最近 N 代。已记进 [TODO](TODO.md) 的 T0.9。
...
- [ ] binlog 的磁盘上限定下来（写满时 MySQL 直接停止写入） —— ⚠️ 2026-09-16 实测：根文件系统 29 GB / 已用 68%，余量约 9 GB；**增长最快的不是 binlog 而是部署镜像**（本项目 16 个、约 3.4 GB，无清理），见 §8.1 末尾
