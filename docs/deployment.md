...
| **备份 / 恢复 / RPO·RTO** | ⚠️ 全量（每日）+ binlog 离机（每 4 分钟）+ PITR 恢复 + cron 已落地，**本地整链演练通过**（§5.2.1、§5.2.3、§5.2.4）；**还没在 VPS 上跑过**，新鲜度监控未做 |
| **生产日志与告警** | ❌ 第 7、8 节 |
...
[`deploy/cron.d/ai_billing_hub`](../deploy/cron.d/ai_billing_hub)：binlog **每 4 分钟**、全量每天一次，
`flock -n` 防叠加，日志进 syslog（`journalctl -t billing-binlog`）。

⚠️ **4 而不是 5**：丢数据的上限是「间隔 + 一次推送的耗时」。
⚠️ 装进 `/etc/cron.d` 而不是 `crontab <file>` —— 后者会**整个替换**该用户的 crontab，
同机其它项目的定时任务一并抹掉。安装命令在那个文件开头。

⚠️ **cron 失败是静默的。**「推送断了三天」只会表现为某一轮报断档 —— 所以 §5.2 的
新鲜度监控仍然是上线前必须补的，这里没有做。
