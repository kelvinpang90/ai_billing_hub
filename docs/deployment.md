...
| **备份 / 恢复 / RPO·RTO** | ⚠️ 全量（每日）+ binlog 离机（每分钟，破 RPO 以 err 级别报出）+ PITR 恢复 + cron 已落地，**本地整链演练通过**（§5.2.1、§5.2.3–§5.2.5）；**2026-09-14 在 VPS 上实测 binlog 离机 + PITR 恢复通过**（§5.2.4 末尾），主密钥恢复未演练、每日全量的 cron 尚未见到自动跑过，告警通道未做 |
| **生产日志与告警** | ❌ 第 7、8 节 |
...
这次**没有**覆盖的：主密钥恢复（§6、§11）；§98.1 要求的钱包 / 账本 / 用量 / 支付 / 对账单 / 文档
...
- [x] 进程模型定案（保持单进程，见 §3）
- [ ] 备份四层全部在跑，且有新鲜度监控
- [ ] **做过一次恢复演练**，含主密钥恢复，并记录实际耗时 vs RTO 4 小时 —— 数据库那一半 2026-09-14 在 VPS 上通过（60 秒，与生产逐表校验和一致，见 §5.2.4 末尾）；**主密钥恢复还没演练**
