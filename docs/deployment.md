...
- [x] 进程模型定案（保持单进程，见 §3）
- [ ] 备份四层全部在跑，且有新鲜度监控
- [x] **做过一次恢复演练**，含主密钥恢复，并记录实际耗时 vs RTO 4 小时 —— 2026-09-14 在 VPS 上：数据库 60 秒恢复、与生产逐表校验和一致；`master.key` 与备份口令的离线副本指纹与线上一致（Kelvin 拍板据此勾上；密钥部分是指纹比对，未在干净机器上走完整解密流程，见 §5.2.4 末尾）
