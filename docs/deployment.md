...
| **备份 / 恢复 / RPO·RTO** | ⚠️ 全量（每日）+ binlog 离机（每分钟，破 RPO 以 err 级别报出）+ PITR 恢复 + cron 已落地，**本地整链演练通过**（§5.2.1、§5.2.3–§5.2.5）；**2026-09-14 在 VPS 上实测 binlog 离机 + PITR 恢复通过**（§5.2.4 末尾），主密钥与备份口令的离线副本指纹比对一致；每日全量的 cron 尚未见到自动跑过，告警通道未做 |
| **生产日志与告警** | ❌ 第 7、8 节 |
...
所以 §11 那条**还不勾**：钥匙对不等于步骤对（文件格式、属主、挂载路径、重启顺序），而 RTO 算的正是步骤。
...
- [x] 进程模型定案（保持单进程，见 §3）
- [ ] 备份四层全部在跑，且有新鲜度监控
- [ ] **做过一次恢复演练**，含主密钥恢复，并记录实际耗时 vs RTO 4 小时 —— 2026-09-14 在 VPS 上：数据库 60 秒恢复、与生产逐表校验和一致；`master.key` 与备份口令的离线副本指纹与线上一致（§5.2.4 末尾）。⚠️ **还差一次真解密**：只凭离线副本的 `master.key` 解开恢复库里的一条密文。指纹比对证明钥匙对，不证明恢复步骤走得通（Kelvin 2026-09-14 选 B，见 [REVIEW-LOG](REVIEW-LOG.md)）
