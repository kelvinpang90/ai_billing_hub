# AIH-TASK-010 设计：管理端手工调账（已批准 v1）

> **来源**：设计闸门 Issue #111。本文件是 `APPROVED: design v1` 那一版正文的**逐字副本**，审查者为 Codex。
> 放进仓库，是因为 OpenClaw Worker 在沙箱里不联网、读不到 GitHub Issue。
> **实现以本文件为准**；与 Issue 不一致时，以 Issue #111 上被批准的 v1 为准。设计要改，就回到 Issue 升版本、重新过闸门，不要直接改本文件。
> 设计编号沿用 AIH-TASK-010（写出这份草稿的那次 run 结算为 `failed:review_changes_requested`），实现登记为 `AIH-TASK-011`。`amount` 带符号（§9 第一行）已由 Kelvin 于 2026-09-24 确认。
...
目标 PR：设计草稿在 #110；实现 PR 待开（批准后另行登记实现任务）
