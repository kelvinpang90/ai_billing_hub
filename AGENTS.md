...
由 `scripts/codex-review.ps1` 的提示词给定，每次调起都随审查材料一并送达，照它写。

本文件不复述一份：同一个格式约定只准有一份实现，两份迟早分叉，而分叉的那一刻两边都不会报错（见 `docs/REVIEW-LOG.md`）。

格式归提示词管，**内容底线归这里管**。每条 Finding 都要能答出这五样：
...
没有 Blocking Finding 时，照提示词的要求写「无」。
